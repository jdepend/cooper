...
public class ComponentProfile implements Serializable {
	...
	public String getExplain() {
		...
		info.append("组件稳定性是由组件关系计算得到的（I=Ce/(Ca+Ce)）。组件关系有数量和强度两个维度，用户可以通过设置采用数量、强度，或者数量和强度来计算组件的稳定性。\n\n");
	
		info.append("组件内聚性的计算依赖将何种粒度的元素作为子元素。系统提供两种选项，分别是包和类。当采用包作为子元素的时候，组件内包之间的关系越紧密内聚性越强，同理，采用类作为子元素的时候，组件内类之间的关系越紧密内聚性越强。\n\n");
