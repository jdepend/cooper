...
public final class SummaryDomainAnalysis extends AbstractDomainAnalysis {
	...
	protected AdviseInfo doAdvise(String name, AnalysisResult result) {
		...
		Float relationRationality = result.calRelationRationality();
		...
		Float balance = result.calBalance();
		if (balance < 10F) {
		...
		Float encapsulation = result.calEncapsulation();
