...
public class InheritTreeAnalyse extends AbstractAnalyzer {
	...
	public String getExplain() {
		...
		explain
				.append("&nbsp;&nbsp;&nbsp;&nbsp;“宽而浅”的结构比较简单，多数子类继承的父类往往是技术方面的复用。“窄而深”则是对业务逻辑进行了深入分析，抽取了较为细致的复用单元，但存在过度的可能性以及容易出现面对需求变化修改困难的局面，故“窄而深”的结构需要对业务做较为深入的交流。<br>");
		explain.append("<br>");
		...
		explain
				.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4、在继承树上一般都会应用模板模式来规范不同逻辑职责单元的调用顺序。并预留出一些增加新逻辑的地点。对于抽象了核心执行顺序的方法一般都通过final关键字来避免子类覆盖。<br>");
		explain
				.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5、为了在继承树上避免修改父类或子类方法签名而丢失多态效果的情况发生，在JDK1.5以上都会使用Override注解来引起修改者的注意，并在修改这些方法签名时报编译错误。<br>");
