...
public class ComponentInterfaceCheck extends AbstractAnalyzer {
	...
	protected void doSearch(AnalysisResult result) throws JDependException {
		...
		for (JavaClass javaClass : result.getClasses()) {
			for (JavaClass ceClass : javaClass.getEfferents()) {
		...
		// 收集未使用的在接口位置存放的类
		List<JavaClass> privateElements = new ArrayList<JavaClass>();
		for (JavaClass javaClass : result.getClasses()) {
		...
		if (privateElements.size() > 0) {
			...
			for (JavaClass javaClass : privateElements) {
				this.printTab();
	...
	private boolean isAttention(JavaClass javaClass) {
