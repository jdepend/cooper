...
public final class MoveRelationForChangeDirTODOItem extends MoveRelationTODOItem {
	...
	protected boolean decision() throws JDependException {
		if (!this.getRelationData().getRelation().getAttentionType().equals(Relation.ComponentLayerAttentionType)
				&& !this.getRelationData().getRelation().getAttentionType().equals(Relation.SDPAttentionType)
				&& !this.getRelationData().getRelation().getAttentionType().equals(Relation.CycleDependAttentionType)) {
	...
	public String getAccording() {
		return this.getRelationData().getRelation().getAttentionType().equals(Relation.ComponentLayerAttentionType) ? "下层组件依赖了上层组件"
				: (this.getRelationData().getRelation().getAttentionType().equals(Relation.SDPAttentionType) ? "违反稳定依赖原则"
						: "该关系在循环依赖链上表现为最弱的关系");
	}
