...
public class JavaClassRelationCreator {
	...
	public void create(Map<String, JavaClass> javaClassesForName) {
		...
		JavaClassRelationTypeMgr mgr = JavaClassRelationTypeMgr.getInstance();
		Collection<String> createRelationTypes = this.conf.getCreateRelationTypes();
		Collection<String> returnTypes;
		for (JavaClass javaClass : javaClasses) {
			if (javaClass.isInner()) {
				LogUtil.getInstance(JavaClassRelationCreator.class).systemLog("开始建立Class的关系:" + javaClass.getName());
				...
				// 处理参数
				if (createRelationTypes.contains(JavaClassRelationTypeMgr.Param) && info.getParamTypes().size() != 0) {
				...
				// 处理Table关系
				if (createRelationTypes.contains(JavaClassRelationTypeMgr.Table) && info.getTables().size() != 0) {
						...
						if (!tableInfo.isDefine()) {
							...
							if (!JavaClassRelationTypeMgr.getInstance().isIgnoreTableInfo(tableInfo)) {
								...
								for (JavaClass dependJavaClass1 : dependJavaClasses) {
									setDependInfo(javaClass, dependJavaClass1, mgr.getTableRelation().clone(
