...
public class VisualGraph extends Graph implements VisualTupleSet {
	 ...
	 * @param directed
	 *            indicates if graph edges are directed or undirected
	 * @param nodeKey
	 *            the node table field by which to index the nodes. This value
	 *            can be null, indicating that just the row indices should be
	 *            used.
	 * @param sourceKey
	 *            the edge table field storing source node keys
	 * @param targetKey
	 *            the edge table field storing target node keys
	 */
	public VisualGraph(VisualTable nodes, VisualTable edges, boolean directed, String nodeKey, String sourceKey,
			String targetKey) {
		super(nodes, edges, directed, nodeKey, sourceKey, targetKey);
	}

	/**
	 * Fire a graph event. Makes sure to invalidate all edges connected to a
	...
	/**
	 * Get the node row index value for the given key. TODO: test this more
	 * thoroughly?
	 */
