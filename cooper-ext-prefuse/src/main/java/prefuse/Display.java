...
public class Display extends JComponent {
		...
		synchronized (m_vis) {
			synchronized (this) {
				...
				if (m_damageRedraw) {
					...
					m_rclip
							.setFrameFromDiagonal(m_clip.getMinX(), m_clip.getMinY(), m_clip.getMaxX(), m_clip
									.getMaxY());
					g2D.setClip(m_rclip);
					...
					m_rclip.setFrameFromDiagonal(m_clip.getMinX() - pixel, m_clip.getMinY() - pixel, m_clip.getMaxX()
							+ pixel, m_clip.getMaxY() + pixel);
	...
	private class TransformActivity extends Activity {
		...
		protected void run(long elapsedTime) {
			...
			damageReport();
			m_transform.setTransform(src[0] + f * (dst[0] - src[0]), src[1] + f * (dst[1] - src[1]), src[2] + f
					* (dst[2] - src[2]), src[3] + f * (dst[3] - src[3]), src[4] + f * (dst[4] - src[4]), src[5] + f
					* (dst[5] - src[5]));
			try {
				m_itransform = m_transform.createInverse();
		...
		if (m_bounders != null && m_bounders.size() > 0) {
			...
			for (int i = 0; i < lstnrs.length; ++i) {
				try {
					...
					s_logger
							.warning("Exception thrown by ItemBoundsListener: " + e + "\n" + StringLib.getStackTrace(e));
				}
			}
		}
	}
