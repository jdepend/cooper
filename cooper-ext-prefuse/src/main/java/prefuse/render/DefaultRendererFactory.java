 ...
 * can be added, by specifying a Predicate to apply and a Renderer to return for
 * matching items. Predicate/Renderer mappings are checked in the order in which
 * they were added to the factory.
 * </p>
 * 
 * <p>
 * If left unspecified, a {@link ShapeRenderer} is used as the default item
 * renderer and an {@link EdgeRenderer} instance is used as the default edge
 * renderer.
 * </p>
 * 
 * <p>
 * For example, the following code snippet creates a new DefaultRendererFactory,
 * changes the default edge renderer to be an EdgeRenderer using curved edges,
 * and adds a new rule which maps items in the group "data" to a text renderer
 * that pulls its text from a field named "label".
 * </p>
 * 
 * <pre>
...
public class DefaultRendererFactory implements RendererFactory {
	 ...
	 * 
	 * @param itemRenderer
	 *            the default item renderer. This is the default for rendering
	 *            all items except EdgeItem instances.
	 * @see EdgeRenderer
	 */
	 ...
	 *            all items except EdgeItem instances.
	 * @param edgeRenderer
	 *            the default edge renderer. This is the default for rendering
	 *            EdgeItem instances.
	 */
	 ...
	 * 
	 * @param r
	 *            the Renderer to use as the default
	 ...
	 * {@link #getRenderer(VisualItem)} whenever there are no matching
	 * predicates and the input item <em>is</em> an EdgeItem. To set the default
	 * renderer for non-EdgeItems, see {@link #setDefaultRenderer(Renderer)}.
	 * 
	 * @param r
	 *            the Renderer to use as the default for EdgeItems
	 ...
	 * order in which they are added, so if an item matches multiple predicates,
	 * the Renderer for the earliest match will be returned.
	 * 
	 * @param p
	 *            a Predicate for testing a VisualItem
	 * @param r
	 *            the Renderer to return if an item matches the Predicate
	 */
	 ...
	 * 
	 * @param predicate
	 *            a String in the prefuse expression language. This String will
	 *            be parsed to create a corresponding Predicate instance.
	 * @param r
	 *            the Renderer to return if an item matches the Predicate
	 */
	...
	/**
	 * Return a Renderer instance for the input VisualItem. The VisualItem is
	 * matched against the registered Predicates, and if a match is found the
	 * corresponding Renderer is returned. Predicate matches are evaluated in
	 * the order in which Predicate/Renderer mappings were added to this
