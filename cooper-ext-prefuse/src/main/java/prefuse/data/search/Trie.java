...
/**
 * A trie data structure for fast-lookup of words based on their prefixes. The
 * name "Trie" is a play on the words "tree" and "retrieval". This class builds
 * a tree structure representing a set of words by their prefixes. It is useful
 * for performing prefix-based searches over large amounts of text in an
 * efficient manner.
 * 
 * @version 1.0
...
public class Trie {
	...
	/**
	 * A TrieNode implementation representing a branch in the tree. The class
	 * maintains a list of characters (the next character in the prefix) and
	 * associated children TrieNodes for each.
	 */
	 ...
	 * 
	 * @param caseSensitive
	 *            true if the index should be case sensitive for indexed words,
	 *            false otherwise.
	 */
	 ...
	 * @param t
	 *            the Tuple associated with the word
	 */
	...
	private void addLeaf(TrieBranch b, TrieLeaf l, int depth) {
		...
		if (i == -1) {
			...
			if (n == null) {
				...
				TrieLeaf nl = (TrieLeaf) n;
				if (i == 0 || (caseSensitive ? nl.word.equals(l.word) : nl.word.equalsIgnoreCase(l.word))) {
	...
	/**
	 * Look up the given word in this Trie. If a match is found, a TrieNode is
	 * returned. This node is the root of a subtree containing all the matches
	 * to the query.
	 * 
	 * @param word
	 *            the word to lookup
	 * @return the TrieNode root of the subtree containing all matches. A null
	 *         value is returned if no match is found.
	 */
