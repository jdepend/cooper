 ...
 * Tuple data fields. This class uses a {@link Trie Trie} data structure to find
 * search results quickly; however, only prefix matches will be identified as
 * valid search matches. Multi-term search queries will result in the union of
 * the results for the individual query terms. That is, Tuples that match any
...
public class PrefixSearchTupleSet extends SearchTupleSet {
	 ...
	 * 
	 * @param caseSensitive
	 *            true if the search routines should be case sensitive, false
	 *            otherwise.
	 */
	...
	/**
	 * Returns the delimiter string used to divide data values and queries into
	 * separate words. By default, the value consists of just whitespace
	 * characters.
	 * 
	 * @return the delimiter string used. This is passed as an argument to a
	 *         {@link java.util.StringTokenizer} instance that will tokenize the
	 *         text.
	...
	/**
	 * Sets the delimiter string used to divide data values and queries into
	 * separate words. By default, the delimiter consists of just whitespace
	 * characters.
	 * 
	 * @param delim
	 *            the delimiter string to use. This is passed as an argument to
	 *            a {@link java.util.StringTokenizer} instance that will
	 *            tokenize the text.
	 * @see java.util.StringTokenizer
	 */
	...
	/**
	 * Searches the indexed Tuple fields for matching string prefixes, adding
	 * the Tuple instances for each search match to this TupleSet. The query
	 * string is first broken up into separate terms, as determined by the
	 * current delimiter string. A search for each term is conducted, and all
	 * matching Tuples are included in the results.
	 * 
	 * @param query
	...
	protected void addString(String s, Tuple t) {
