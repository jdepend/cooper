...
public class BreadthFirstIterator implements Iterator {
	 ...
	 * 
	 * @param n
	 *            the source node from which to begin the traversal
	 * @param depth
	 ...
	 * 
	 * @param o
	 *            Either a source node or iterator over source nodes
	 * @param depth
	 *            the maximum graph distance to traverse
	 * @param traversal
	 *            the traversal type, one of
	...
	protected Iterator getEdges(Node n) {
		return n.edges(); // TODO: add support for all edges, in links only, out
							// links only
	}
	 ...
	 * @param t
	 *            the tuple to lookup
	 * @return the traversal depth of the tuple, or -1 if the tuple has not been
