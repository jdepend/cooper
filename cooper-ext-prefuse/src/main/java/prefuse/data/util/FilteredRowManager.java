...
/**
 * RowManager that additionally manages mappings between the managed rows and
 * those of a parent table.
 * 
...
public class FilteredRowManager extends RowManager {
	...
	public int getColumnRow(int row, int col) {
		return ((CascadedTable) m_table).getParentTable().getColumnRow(getParentRow(row), col);
	}
	...
	/**
	 * Given a row managed by this manager, return the corresponding row in the
	 * parent table.
	 * 
	 * @param childRow
	...
	/**
	 * Given a row in the parent table, return the corresponding row managed by
	 * this manager.
	 * 
	 * @param parentRow
