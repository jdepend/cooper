...
/**
 * <p>
 * Utility class representing sorting criteria, this can be given as input to
 * the {@link TupleSet#tuples(Predicate, Sort)} method to get a sorted iteration
 * of tuples.
 * </p>
 * 
 * <p>
 * Sort criteria consists of an ordered list of data field names to sort by,
 * along with an indication to sort tuples in either ascending or descending
 * order. These criteria can be passed in to the constructor or added
 * incrementally using the {@link #add(String, boolean)} method.
 * </p>
 * 
 * <p>
 * Alternatively, one can also specify the sorting criteria using a single
 * string, which is parsed using the {@link #parse(String)} method. This string
 * should consist of a comma-delimited list of field names, which optional "ASC"
 * or "DESC" modifiers to specify ascending or descending sorts. If no modifier
 * is given, ascending order is assumed. Field names which include spaces or
 * other non-standard characters should be written in brackets ([]), just as is
 * done in {@link prefuse.data.expression.parser.ExpressionParser expression
 ...
 * 
 * <p>
 * sorts first by the data field "Profit" in descending order, additionally
 * sorting in ascending order by the data field "Product Type" for tuples which
 * have identical values in the "Profit" field.
 * </p>
 * 
...
public class Sort {
	...
	/**
	 * Creates a new Sort specification that sorts on the given fields, all in
	 * ascending order.
	 * 
	 * @param fields
	...
	/**
	 * Creates a new Sort specification that sorts on the given fields in the
	 * given orders.
	 * 
	 * @param fields
	 *            the fields to sort on, in order of precedence
	 * @param ascend
	 *            for each field, indicates if the field should be sorted in
	 ...
	 * 
	 * @param field
	 *            the additional field to sort on
	 * @param ascend
	 *            indicates if the field should be sorted in ascending (true) or
	 ...
	 * @param i
	 *            the index to look up
	 * @return true if the field at the given index is to be sorted in ascending
	...
	/**
	 * Generates a Comparator to be used for sorting tuples drawn from the given
	 * tuple set.
	 * 
	 * @param ts
	 *            the TupleSet whose Tuples are to be sorted
	 * @return a Comparator instance for sorting tuples from the given set using
	 *         the sorting criteria given in this specification
	 */
	...
	/**
	 * Parse a comma-delimited String of data fields to sort on, along with
	 * optional ASC or DESC modifiers, to generate a new Sort specification.
	 * This string should consist of a comma-delimited list of field names,
	 * which optional "ASC" or "DESC" modifiers to specify ascending or
	 * descending sorts. If no modifier is given, ascending order is assumed.
	 * Field names which include spaces or other non-standard characters should
	 ...
	 * <p>
	 * sorts first by the data field "Profit" in descending order, additionally
	 * sorting in ascending order by the data field "Product Type" for tuples
	 * which have identical values in the "Profit" field.
	 * </p>
	 * 
	 * @param s
