 ...
 * column names, data types, and default values. New Table instances can be
 * created directly from Schema objects through the use of the
 * {@link #instantiate()} method. If a schema is subsequently changed,
 * instantiated table instances are not affected, keeping their original schema.
 * </p>
 * 
 * <p>
...
public class Schema implements Cloneable {
	...
	public void addColumn(String name, Class type, Object defaultValue) {
		...
		for (int i = 0; i < m_size; ++i) {
			if (m_names[i].equals(name)) {
				throw new IllegalArgumentException("Duplicate column names are not allowed: " + m_names[i]);
			}
		}
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < m_size; ++i) {
			if (!(m_names[i].equals(s.getColumnName(i)) && m_types[i].equals(s.getColumnType(i)) && m_dflts[i].equals(s
					.getDefault(i)))) {
				return false;
			}
		}
