...
/**
 * Tuple sub-interface that represents a node in a graph or tree structure. This
 * interface supports both graph and tree methods, tree methods invoked on a
 * node in a general graph typically default to operations on the graph's
...
public interface Node extends Tuple {
	...
	/**
	 * Get the in-degree of the node, the number of edges for which this node is
	 * the target.
	 * 
	...
	/**
	 * Get the degree of the node, the number of edges for which this node is
	...
	/**
	 * Get an iterator over all incoming edges, those for which this node is the
	 * target.
	 * 
	...
	/**
	 * Get an iterator over all outgoing edges, those for which this node is the
	 * source.
	 * 
	...
	/**
	 * Get an iterator over all incident edges, those for which this node is
	 * either the source or the target.
	 * 
	 ...
	 * 
	 * @return the tree depth of this node. The root's tree depth is zero, and
	 *         each level of the tree is one depth level greater.
	 */
	public int getDepth();
	 ...
	 * @return the index of the child node, or -1 if the node is not a child of
	 *         this one.
	 */
	...
	/**
	 * Get this node's first tree child. This is the same as looking up the node
	 * at index 0.
	 * 
	...
	/**
	 * Get this node's last tree child. This is the same as looking up the node
	 * at the child count minus 1.
	 * 
