...
/**
 * <p>
 * Graph subclass that models a tree structure of hierarchical parent-child
 * relationships. For each edge, the source node is considered the parent, and
 * the target node is considered the child. For the tree structure to be valid,
 * each node can have at most one parent, and hence only one edge for which that
 * node is the target. In addition to the methods of the Graph class, the tree
 * also supports methods for navigating the tree structure, such as accessing
 * parent or children nodes and next or previous sibling nodes (siblings are
 * children nodes with a shared parent). Unlike the graph class, the default
 * source and target key field names are renamed to {@link #DEFAULT_SOURCE_KEY}
 * and {@link #DEFAULT_TARGET_KEY}. Like the {@link Graph} class, Trees are
 * backed by node and edge tables, and use {@link prefuse.data.Node} and
 * {@link prefuse.data.Edge} instances to provide object-oriented access to
 * nodes and edges.
 * </p>
 * 
 * <p>
 * The Tree class does not currently enforce that the graph structure remain a
 * valid tree. This is to allow a chain of editing operations that may break the
 * tree structure at some point before repairing it. Use the
 * {@link #isValidTree()} method to test the validity of a tree.
 * </p>
 * 
 * <p>
 * By default, the {@link #getSpanningTree()} method simply returns a reference
 * to this Tree instance. However, if a spanning tree is created at a new root u
 * sing the {@link #getSpanningTree(Node)} method, a new {@link SpanningTree}
...
public class Tree extends Graph {
	...
	/**
	 * Add a child node to the given parent node. An edge between the two will
	 * also be created.
	 * 
	 * @param parent
	...
	/**
	 * Remove a child edge from the Tree. The child node and its subtree will
	 * also be removed from the Tree.
	 * 
	 * @param edge
	 ...
	 * @return true if the node and its subtree is successfully removed, false
	 *         otherwise
	 */
	...
	/**
	 * Get the child index (order number of the child) for the given parent node
	 * id and child node id.
	 * 
	 * @param parent
	 *            the parent node id (node table row number)
	 * @param child
	 ...
	 * @return the node id of the previous sibling, or -1 if there is no
	 *         previous sibling.
	 */
	 ...
	 * @param node
	 *            a node id (node table row number)
	 * @return the node id of the next sibling, or -1 if there is no next
	 *         sibling.
	 */
	 ...
	 * @param node
	 *            a node id (node table row number)
	 * @return the depth of the node in tree. The root node is at a depth level
	 *         of 0, with each child at a greater depth level. -1 is returned if
	 *         the input node id is not in the tree.
	 */
	public int getDepth(int node) {
		...
		int depth = 0;
		if (node != m_root && getParent(node) < 0)
			return -1;
	...
	/**
	 * Get an iterator over the edge ids for edges connecting child nodes to a
	 * given parent
	 * 
	 * @param node
	 *            the parent node id (node table row number)
	 * @return an iterator over the edge ids for edges conencting child nodes to
	 *         a given parent
	 */
	...
	public boolean isValidTree() {
		...
		if (counts[0] < nedges) {
			s_logger.warning("Not all of the tree was visited. " + "Only " + counts[0] + "/" + nedges
					+ " edges encountered");
			return false;
		}
		return true;
	...
	/**
	 * Returns a spanning tree over this tree. If no spanning tree has been
	 * constructed at an alternative root, this method simply returns a pointer
	 * to this Tree instance. If a spanning tree rooted at an alternative node
	 * has been created, that tree is returned.
	 * 
	...
	/**
	 * Returns a spanning tree over this tree, rooted at the given root. If the
	 * given root is not the same as that of this Tree, a new spanning tree
	 * instance will be constructed, made the current spanning tree for this
	 * Tree instance, and returned.
	 * 
	 ...
	 * getSpanningTree() method (with no arguments) will return a pointer to
	 * this Tree instance instead of any generated spanning trees.
	 * 
	 * @param root
