...
/**
 * TableWriter for fixed-width text files, that encode one row of table data per
 * line use a fixed number of characters for each data column. Writing such
 * tables requires use of a schema description that describes the fixed-widths
 * for each individual column. The
 * {@link prefuse.data.io.FixedWidthTextTableSchema} class provides this
 * functionality. A schema description must be written separately into a
 * different file.
 * 
...
public class FixedWidthTextTableWriter extends AbstractTableWriter {
	 ...
	 * 
	 * @param schema
	 *            the schema description of the fixed-width text column lengths
	 */
	...
	/**
	 * Creates a new FixedWidthTextTableWriter using the schema at the given
	 * location.
	 * 
	 * @param location
	 *            a location string (filename, URL, or resource locator) for the
	 *            schema description of the fixed-width text column lengths
	 * @throws DataIOException
	 *             if an IO exception occurs while loading the schema
	 */
	...
	/**
	 * @see prefuse.data.io.TableWriter#writeTable(prefuse.data.Table,
	...
	public void writeTable(Table table, OutputStream os) throws DataIOException {
		try {
			...
			for (IntIterator rows = table.rows(); rows.hasNext();) {
				...
				for (int i = 0; i < table.getColumnCount(); ++i) {
					out.print(pack(table.getString(row, i), m_schema.getColumnLength(i), pre[i], pad[i]));
				}
				out.println();
			}
