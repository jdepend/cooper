...
/**
 * GraphWriter instance that writes a tree file formatted using the TreeML file
 * format. TreeML is an XML format originally created for the 2003 InfoVis
 * conference contest. A DTD (Document Type Definition) for TreeML is <a
 * href="http://www.nomencurator.org/InfoVis2003/download/treeml.dtd"> available
 * online</a>.
 * 
 * <p>
 * The GraphML spec only supports the data types <code>Int</code>,
 * <code>Long</code>, <code>Float</code>, <code>Real</code> (double),
 * <code>Boolean</code>, <code>String</code>, and <code>Date</code>. An
 * exception will be thrown if a data type outside these allowed types is
 * encountered.
 * </p>
 * 
...
public class TreeMLWriter extends AbstractGraphWriter {
	...
	/**
	 * Find the next node in the depth first iteration, closing off open branch
	 * tags as needed.
	 * 
	 * @param x
	 *            the current node
	 ...
	 * The TreeML spec only allows the types <code>int</code>, <code>long</code>
