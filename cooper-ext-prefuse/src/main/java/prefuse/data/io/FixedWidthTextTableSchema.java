...
/**
 * Helper class that stores character length counts for each column in a fixed
 * width text table. This class is needed for reading and writing fixed width
 * data tables. A schema definition can either be created manually using the
 * {@link #addColumn(String, int)} method or loaded from a tab-delimited text
 * file using the {@link #load(String)} method.
 * 
...
public class FixedWidthTextTableSchema {
	 ...
	 * @param idx
	 *            the index of the column
	 * @return the text character position at which this column starts on a line
	 */
	 ...
	 * 
	 * @param loc
	 *            a location string representing a filename, URL, or resource
	 *            locator
