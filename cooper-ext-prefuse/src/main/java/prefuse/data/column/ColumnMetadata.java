 ...
 * instance. They are created automatically by Table instances and are retrieved
 * using the {@link prefuse.data.Table#getMetadata(String)} method.
 * 
 * @author <a href="http://jheer.org">jeffrey heer</a>
 */
public class ColumnMetadata implements ColumnListener {

	// TODO consider refactor. is non-dynamic mode needed? pass Column reference
	// in?
	 ...
	 *            the backing table
	 * @param column
	 *            the name of the column to store metadata for
	 */
	 ...
	 *            the name of the column to store metadata for
	 * @param cmp
	 *            a Comparator that determines the default sort order for values
	 *            in the column
	 * @param dynamic
	 *            indicates if this ColumnMetadata should react to changes in
	 *            the underlying table values. If true, computed values stored
	 *            in this metadata object will be invalidated when updates to
	...
	public int getMedianRow() {
		...
		if (m_median == -1 && m_dynamic) {
			...
			if (idx != null) {
				...
				m_median = DataLib.median(m_table.tuples(), m_field, m_cmp).getRow();
			}
		}
		return m_median;
	}
	...
	/**
	 * Get the mean value of numeric values in the column. If this column does
	 * not contain numeric values, this method will result in an exception being
	 * thrown.
	 * 
	...
	/**
	 * Get the standard deviation of numeric values in the column. If this
	 * column does not contain numeric values, this method will result in an
	...
	/**
	 * Get the sum of numeric values in the column. If this column does not
	 * contain numeric values, this method will result in an exception being
	 * thrown.
	 * 
	...
	/**
	 * Get a map between all unique column values and their integer index in the
	 * sort order of those values. For example, the minimum value maps to 0, the
	 * next greater value to 1, etc.
	 * 
	 * @return a map between all unique column values and their integer index in
	 *         the values' sort order
	 */
	public Map getOrdinalMap() {
		...
		if (m_ordinalM == null && m_dynamic) {
			...
			// m_ordinalM = DataLib.ordinalMap(m_table.tuples(), m_field,
			// m_cmp);
		}
		return m_ordinalM;
	}
