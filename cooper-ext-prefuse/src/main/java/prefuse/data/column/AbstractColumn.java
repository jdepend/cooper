...
/**
 * Abstract base class for Column implementations. Provides listener support and
 * default implementations of column methods.
 * 
...
public abstract class AbstractColumn implements Column {
	 ...
	 * 
	 * @param readOnly
	 *            true to ensure the values can not be edited, false otherwise
	 */
	...
	public void setParser(DataParser parser) {
		if (!m_columnType.isAssignableFrom(parser.getType())) {
			throw new IllegalArgumentException("Parser type (" + parser.getType().getName() + ") incompatible with"
	...
	/**
	 * Sets the default value for this column. Rows previously added under a
	 * different default value will not be changed as a result of this method;
	 * the new default will apply to newly added rows only.
	 * 
	 * @param dflt
	 */
	public void setDefaultValue(Object dflt) {
		...
		if (dflt != null
				&& ((!prim && !m_columnType.isInstance(dflt)) || (prim && !TypeLib
						.isWrapperInstance(m_columnType, dflt)))) {
			throw new IllegalArgumentException("Default value is not of type " + m_columnType.getName());
		}
		m_defaultValue = dflt;
	}
	...
	/**
	 * Indicates if the get method can be called without an exception being
	 * thrown for the given type.
	 * 
	 * @param type
	...
	public boolean canGet(Class type) {
		...
		if (m_columnType.isPrimitive()) {
			boolean primTypes = type.isAssignableFrom(m_columnType)
					|| (TypeLib.isNumericType(m_columnType) && TypeLib.isNumericType(type));

			return primTypes || type.isAssignableFrom(TypeLib.getWrapperType(m_columnType))
	...
	/**
	 * Indicates if the set method can be called without an exception being
	 * thrown for the given type.
	 * 
	 * @param type
	 *            the Class of the data type to check
	...
	public boolean canSet(Class type) {
		...
		if (m_columnType.isPrimitive()) {
			return m_columnType.isAssignableFrom(type) || TypeLib.getWrapperType(m_columnType).isAssignableFrom(type)
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the int type.
	 * 
	...
	/**
	 * Indicates if convenience set method can be called without an exception
	 * being thrown for the int type.
	 * 
	 ...
	 *             if this column does not support the integer type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the long type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the long type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the float type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the float type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the double type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the double type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the boolean type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the boolean type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the String type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the String type
	 */
	...
	/**
	 * Indicates if convenience get method can be called without an exception
	 * being thrown for the Date type.
	 * 
	 ...
	 * @throws DataTypeException
	 *             if this column does not support the Date type
	 */
