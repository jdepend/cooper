...
/**
 * <p>
 * Table subclass featuring a "cascaded" table design - a CascadedTable can have
 * a parent table, from which it inherits a potentially filtered set of rows and
 * columns. Child tables may override the columns of the parent by having a
 * column of the same name as that of the parent, in which case the parent's
 * column will not be accessible.
 * </p>
 * 
 * <p>
 * Table rows of the parent table can be selectively included by providing a
 * {@link prefuse.data.expression.Predicate} that filters the parent rows.
 * Columns of the parent table can be selectively included by providing a
 * {@link prefuse.data.util.ColumnProjection} indicating the columns to include.
 * </p>
 * 
 * <p>
 * Tuple instances backed by a CascadedTable will be not be equivalent to the
 * tuples backed by the parent table. However, setting a value in a
 * CascadedTable that is inherited from a parent table <em>will</em> update the
...
public class CascadedTable extends Table {
	...
	/**
	 * ColumnProjection determining which columns of the parent table are
	 * included in this table.
	 */
	protected ColumnProjection m_colFilter;
	/**
	 * Selection Predicate determining which rows of the parent table are
	 * included in this table.
	 */
	...
	/**
	 * Create a new CascadedTable. By default all rows and columns of the parent
	 * table are included in this one.
	 * 
	 * @param parent
	 *            the parent Table to use
	 */
	...
	/**
	 * Create a new CascadedTable. By default all columns of the parent table
	 * are included in this one.
	 * 
	 * @param parent
	 *            the parent Table to use
	 * @param rowFilter
	 *            a Predicate determining which rows of the parent table to
	 *            include in this one.
	 */
	...
	/**
	 * Create a new CascadedTable. By default all rows of the parent table are
	 * included in this one.
	 * 
	 * @param parent
	 *            the parent Table to use
	 * @param colFilter
	 *            a ColumnProjection determining which columns of the parent
	 ...
	 *            table to include in this one.
	 * @param tupleType
	 *            the class type of the Tuple instances to use
	 */
	 ...
	 * filtering predicate concerns only items within this table, calling this
	 * method should be unnecessary. It is only when the filtering predicate
	 * references data outside of this table that a manual re-filtering request
	 * may be necessary. For example, filtering valid edges of a graph from a
	 * pool of candidate edges will depend on the available nodes.
	 * 
	...
	public void filterRows() {
		...
		while (crows.hasNext()) {
			...
			if (!m_rowFilter.getBoolean(m_parent.getTuple(rowman.getParentRow(crow)))) {
	...
	/**
	 * Get the ColumnProjection determining which columns of the parent table
	 * are included in this one.
	 * 
	...
	/**
	 * Sets the ColumnProjection determining which columns of the parent table
	 * are included in this one.
	 * 
	 * @param colFilter
	 *            a ColumnProjection determining which columns of the parent
	...
	/**
	 * Gets ths Predicate determining which rows of the parent table are
	 * included in this one.
	 * 
	...
	/**
	 * Sets the Predicate determining which rows of the parent table are
	 * included in this one.
	 * 
	 * @param rowFilter
	 *            a Predicate determining which rows of the parent table to
	...
	/**
	 * Given a row in the parent table, return the corresponding row, if any, in
	 * this table.
	 * 
	 * @param prow
	...
	/**
	 * Internal method for adding a new cascaded row backed by the given parent
	 * row.
	 * 
	 * @param prow
	...
	protected Iterator getColumnNames() {
		if (m_parent == null) {
			...
			return new CompositeIterator(m_names.iterator(), m_pnames.iterator());
		}
	}
