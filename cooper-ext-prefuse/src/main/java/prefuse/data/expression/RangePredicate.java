...
/**
 * Predicate instance that evaluates if a value is contained within a bounded
 * range.
 * 
...
public class RangePredicate extends BinaryExpression implements Predicate {
	...
	public RangePredicate(Expression middle, Expression left, Expression right) {
		this(IN_IN, middle, left, right, DefaultLiteralComparator.getInstance());
	}
	 ...
	 *            the upper range bound
	 * @param cmp
	 *            the comparator to use for comparing data values
	 */
	 ...
	 * 
	 * @param operation
	 *            operation code indicating the inclusiveness / exclusiveness of
	 *            the bounds
	 * @param middle
	 *            the value to test for membership in the range
	 * @param left
	...
	public String toString() {
		...
		return '(' + m_left.toString() + ' ' + lop + ' ' + m_middle.toString() + " AND " + m_middle.toString() + ' '
				+ rop + ' ' + m_right.toString() + ')';
	}
