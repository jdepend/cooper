...
/**
 * Factory class that maintains a collection of parser instances and returns the
 * appropriate parser based on a history of samples presented to the factory.
 * The {@link #sample(String)} method takes a text string and tests it against
 * all available parsers, updating whether or not the parsers can successfully
 * parse the value. This method is used in a more automated fashion by the
 * {@link TypeInferencer} class.
 * 
...
public class ParserFactory implements Cloneable {
	...
	private static final DataParser[] DEFAULT_PARSERS = new DataParser[] { new IntParser(), new LongParser(),
			new DoubleParser(), new FloatParser(), new BooleanParser(), new ColorIntParser(), new DateParser(),
			new TimeParser(), new DateTimeParser(), new IntArrayParser(), new LongArrayParser(),
			new FloatArrayParser(), new DoubleArrayParser(), new StringParser() };
	 ...
	 * following data types (in the provided order of precedence): int, long,
	 * double, float, boolean, Date, Time, DateTime, String.
	 * 
	 ...
	 * following data type in the followinf order of precedence: int, long,
	 * double, float, boolean, Date, Time, DateTime, String.
	 */
	 ...
	 * After a series of samples, the <code>getParser()</code> method can be
	 * used to retrieve the highest ranking candidate parser.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * The ordering of parsers in the array is taken to be the desired order of
	 * precendence of the parsers. For example, if both parser[0] and parser[2]
	 * can parse all the available samples, parser[0] will be returned.
	 * </p>
	 * 
	 * @param parsers
	 *            the input DataParsers to use.
	 */
	...
	/**
	 * Returns the highest ranking parser that successfully can parse all the
	 * input samples viewed by this instance. If no such parser exists, a null
	 * value is returned.
	 * 
	 ...
	 * 
	 * @param type
	 *            the Class for the data type to parse
	 * @return a parser for the given data type, or null if no such parser can
	 *         be found.
	 */
	...
	/**
	 * Analyzes the given array of String values to determine an acceptable
	 * parser data type.
	 * 
	 * @param data
	 *            an array of String values to parse
	 * @param startRow
	 *            the row from which to begin analyzing the data array, allowing
	 *            header rows to be excluded.
	 * @return the appropriate parser for the inferred data type, of null if
	 *         none.
	 */
	...
	/**
	 * Analyzes a column of the given array of String values to determine an
	 * acceptable parser data type.
	 * 
	 * @param data
	 *            an 2D array of String values to parse
	 * @param col
	 *            an index for the column to process
	 * @param startRow
	 *            the row from which to begin analyzing the data array, allowing
	 *            header rows to be excluded.
	 * @return the appropriate parser for the inferred data type, of null if
