...
/**
 * <p>
 * Abstract base class for dynamic query bindings, which support data queries
 * that can be dynamically edited with direct manipulation user interface
 * components. DynamicQueryBinding instances take a particular field of a table,
 * create a {@link prefuse.data.expression.Predicate} instance for filtering
 * Tuples based on the values of that data field, and bind that Predicate to any
 * number of user interface components that can be used to manipulate the
 * parameters of the predicate.
 * </p>
 * 
 * <p>
 * Examples include dynamically filtering over a particular range of values (
 * {@link RangeQueryBinding}), isolating specific categories of data (
 * {@link ListQueryBinding}), and performing text search over data (
 * {@link SearchQueryBinding}).
 * </p>
 * 
...
public abstract class DynamicQueryBinding {
	 ...
	 * @param field
	 *            the data field (Table column) to query
	 */
	...
	/**
	 * Generates a new user interface component for dynamically adjusting the
	 * query values. The type of the component depends on the subclass of
	 * DynamicQueryBinding being used. Some subclasses can generate multiple
	 * types of user interface components. Such classes will include additional
	 * methods for generating the specific kinds of components supported.
	 * 
