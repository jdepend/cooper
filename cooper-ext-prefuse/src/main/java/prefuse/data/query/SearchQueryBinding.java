 ...
 * {@link prefuse.data.search} package can be used to control the type of search
 * index used.
 * 
...
public class SearchQueryBinding extends DynamicQueryBinding {
	...
	/**
	 * Create a new SearchQueryBinding over the given set and data field, using
	 * the specified SearchTupleSet instance. Use this constructor to choose the
	 * type of search engine used, and to potentially reuse the same search set
	 * over multiple dynamic query bindings.
	 * 
	 * @param ts
	 *            the TupleSet to query
	 * @param field
	 *            the data field (Table column) to query
	 * @param set
	 *            the {@link prefuse.data.search.SearchTupleSet} to use.
	 */
	 ...
	 *            if true, each keystroke will cause the search to be re-run
	 *            (this is the default for prefix searches); if false, searches
	 *            will only re-run when the enter key is typed (this is the
	 *            default for the other search engine types).
	 * @return a {@link prefuse.util.ui.JSearchPanel} bound to this dynamic
	 *         query.
	 */
