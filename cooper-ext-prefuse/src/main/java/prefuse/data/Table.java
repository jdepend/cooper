 ...
 * containing a data record, and each column containing data values for a named
 ...
 * {@link prefuse.data.tuple.TupleSet} interface.
 * </p>
 * 
 * <p>
 * Table rows can be inserted or deleted. In any case, none of the other
 * existing table rows are effected by an insertion or deletion. A deleted row
 * simply becomes invalid--any subsequent attempts to access the row either
 * directly or through a pre-existing Tuple instance will result in an
 * exception. However, if news rows are later added to the table, the row number
 * for previously deleted nodes can be reused. In fact, the lower row number
 * currently unused is assigned to the new row. This results in an efficient
 * reuse of the table rows, but carries an important side effect -- rows do not
 * necesarily maintain the order in which they were added once deletions have
 * occurred on the table. If not deletions occur, the ordering of table rows
 * will reflect the order in which rows were added to the table.
 * </p>
 * 
 * <p>
 * Collections of table rows can be accessed using both iterators over the
 * actual row numbers and iterators over the Tuple instances that encapsulate
 * access to that row. Both types of iteration can also be filtered by providing
 * a {@link prefuse.data.expression.Predicate}, allowing tables to be queried
 * for specific values.
 * </p>
 * 
 * <p>
 * Columns (alternativele referred to as data fields) can be added to the Table
 * using {@link #addColumn(String, Class)} and a host of similar methods. This
 * method will automatically determine the right kind of backing column instance
 * to use. Furthermore, Table columns can be specified using a {@link Schema}
 ...
 * a read-only way) using the {@link #getSchema()} method.
 * </p>
 * 
 * <p>
 * Tables also support additional structures. The {@link ColumnMetadata} class
 * returned by the {@link #getMetadata(String)} method supports calculation of
 * different statistics for a column, including minimum and maximum values, and
 * the number of unique data values in the column.
 * {@link prefuse.data.util.Index} instances can be created and retrieved using
 * the {@link #index(String)} method and retrieved without triggering creation
 * using {@link #getIndex(String)} method. An index keeps a sorted collection of
 * all data values in a column, accelerating the creation of filtered iterators
 * by optimizing query calculations and also providing faster computation of
 * many of the {@link ColumnMetadata} methods. If you will be issuing a number
 * of queries (i.e., requesting filtered iterators) dependent on the values of a
 * given column, indexing that column may result in a significant performance
 * increase, though at the cost of storing and maintaining the backing index
 * structure.
 * </p>
 * 
...
public class Table extends AbstractTupleSet implements ColumnListener {
	...
	/**
	 * Mapping between column names and column entries containing column,
	 * metadata, and index references
	 */
	...
	/**
	 * Memoize the index of the last column operated on, used to expedite
	 * handling of column updates.
	 */
	...
	/**
	 * Create a new Table with a given number of rows, and the starting capacity
	 * for a given number of columns.
	 * 
	 * @param nrows
	 *            the starting number of table rows
	 * @param ncols
	 ...
	 *            the starting capacity for columns
	 * @param tupleType
	 *            the class of the Tuple instances to use
	 */
	 ...
	 * deleting rows, and updating table cell values all contribute to this
	 * count.
	 * 
	...
	/**
	 * Sets the TupleManager used by this Table. Use this method carefully, as
	 * it will cause all existing Tuples retrieved from this Table to be
	 * invalidated.
	 * 
	 * @param tm
	 *            the TupleManager to use
	 */
	...
	/**
	 * Returns this Table's schema. The returned schema will be locked, which
	 * means that any attempts to edit the returned schema by adding additional
	 * columns will result in a runtime exception.
	 * 
	 * If this Table subsequently has columns added or removed, this will not be
	 * reflected in the returned schema. Instead, this method will need to be
	 * called again to get a current schema. Accordingly, it is not recommended
	 * that Schema instances returned by this method be stored or reused across
	 * scopes unless that exact schema snapshot is desired.
	 * 
	 ...
	 * corresponding to the given table cell. For basic tables this just returns
	 * the input row value. However, for tables that inherit data columns from a
	...
	/**
	 * Get the row number for this table given a row number for a backing data
	 * column and the column number for the data column. For basic tables this
	 * just returns the column row value. However, for tables that inherit data
	 * columns from a parent table and present a filtered view on this data, a
	 ...
	 * 
	 * @param colrow
	 *            the row of the backing data column
	 * @param col
	 *            the table column to lookup.
	...
	/**
	 * Add a row to this table. All data columns will be notified and will take
	 * on the appropriate default values for the added row.
	 * 
	 ...
	 * notified and will take on the appropriate default values for the added
	 * rows.
	 * 
	 * @param nrows
	 ...
	 * @param row
	 *            the row to delete
	 * @return true if the row was successfully deleted, false if the row was
	 *         already invalid
	 */
	...
	/**
	 * Internal method indicating if the given data field is included as a data
	 * column.
	 */
	 ...
	 *            the data field name for the column
	 * @param expr
	 *            a String expression in the prefuse expression language, to be
	 *            parsed into an {@link prefuse.data.expression.Expression}
	 *            instance. The string is parsed by the
	 *            {@link prefuse.data.expression.parser.ExpressionParser}. If an
	 *            error occurs during parsing, an exception will be thrown.
	 * @see prefuse.data.tuple.TupleSet#addColumn(java.lang.String,
	 *      java.lang.String)
	 */
	...
	/**
	 * Add a constant column to this table, which returns one constant value for
	 * all column rows.
	 * 
	 * @param name
	 *            the data field name for the column
	...
	protected void addColumn(String name, Column col) {
		...
		ColumnEntry entry = new ColumnEntry(m_lastCol, col, new ColumnMetadata(this, name));
	...
	/**
	 * Create (if necessary) and return an index over the given data field. The
	 * first call to this method with a given field name will cause the index to
	 * be created and stored. Subsequent calls will simply return the stored
	 * index. To attempt to retrieve an index without triggering creation of a
	 * new index, use the {@link #getIndex(String)} method.
	 * 
	 * @param field
	 ...
	 * @param field
	 *            the data field name of the column
	 * @return the stored index for the column, or null if no index has been
	 *         created
	 */
	 ...
	 *            the data field name of the column
	 * @param expType
	 *            the expected data type of the index
	 * @param create
	 *            indicates whether or not a new index should be created if none
	 *            currently exists for the given data field
	 ...
	 * @return true if an index was successfully removed, false if no such index
	 *         was found
	 */
	 ...
	 * @return true if the Tuple represents a row of this table, false if it
	 *         does not
	...
	/**
	 * Get the number of tuples in this table. This is the same as the value
	 * returned by {@link #getRowCount()}.
	 * 
	 ...
	 * @param type
	 *            a Class instance to check for compatibility with the data
	 *            field values.
	...
	/**
	 * Check if the <code>set</code> method for the given data field can accept
	 * values of a given target type.
	 * 
	 * @param field
	 *            the data field to check
	 * @param type
	 ...
	 *            field values.
	 * @return true if the data field is compatible with provided type, false
	 *         otherwise. If the value is true, objects of the given type can be
	 *         used as parameters of the {@link #set(int, String, Object)}
	 *         method.
	 ...
	 * 
	 * @param row
	 *            the table row to get
	 * @param field
	 *            the data field to retrieve
	 * @return the data value as an Object. The concrete type of this Object is
	 *         dependent on the underlying data column used.
	 ...
	 * @param val
	 *            the value for the field. If the concrete type of this Object
	 *            is not compatible with the underlying data model, an Exception
	 *            will be thrown. Use the {@link #canSet(String, Class)} method
	 *            to check the type-safety ahead of time.
	 ...
	 * @param field
	 *            the data field
	 * @return the default value, as an Object, used to populate rows of the
	 *         data field.
	 */
	...
	/**
	 * Revert this tuple's value for the given field to the default value for
	 * the field.
	 * 
	 * @param field
	 *            the data field
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getInt(int, String)} method can be used safely.
	 */
	...
	/**
	 * Check if the <code>setInt</code> method can safely be used for the given
	 * data field.
	 * 
	 * @param field
	 *            the data field to check
	...
	/**
	 * Get the data value at the given row and field as an <code>int</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	 *            the data field to retrieve
	...
	/**
	 * Set the data value of the given row and field as an <code>int</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 *            the table row to retrieve
	 * @param col
	 *            the column number of the data field to retrieve
	 * @see #canGetInt(String)
	 */
	 ...
	 *            the table row to set
	 * @param col
	 *            the column number of the data field to set
	 * @param val
	 *            the value to set
	 * @see #canSetInt(String)
	 */
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getLong(int, String)} method can be used safely.
	 */
	...
	/**
	 * Check if the <code>setLong</code> method can safely be used for the given
	 * data field.
	 * 
	 * @param field
	 *            the data field to check
	 * @return true if the {@link #setLong(int, String, long)} method can safely
	 *         be used for the given field, false otherwise.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>long</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>long</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	...
	/**
	 * Get the data value at the given row and field as an <code>long</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param col
	...
	/**
	 * Set the data value of the given row and field as an <code>long</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param col
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getFloat(int, String)} method can be used safely.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>float</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>float</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 *            the table row to retrieve
	 * @param col
	 *            the column number of the data field to get
	 * @see #canGetFloat(String)
	 */
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getDouble(int, String)} method can be used safely.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>double</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>double</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getBoolean(int, String)} method can be used safely.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>boolean</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>boolean</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getString(int, String)} method can be used safely.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>String</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>String</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 *         values, false otherwise. If true, the
	 *         {@link #getDate(int, String)} method can be used safely.
	 */
	...
	/**
	 * Check if the <code>setDate</code> method can safely be used for the given
	 * data field.
	 * 
	 * @param field
	 *            the data field to check
	 * @return true if the {@link #setDate(int, String, Date)} method can safely
	 *         be used for the given field, false otherwise.
	 */
	...
	/**
	 * Get the data value at the given row and field as a <code>Date</code>.
	 * 
	 * @param row
	 *            the table row to retrieve
	 * @param field
	...
	/**
	 * Set the data value of the given row and field as a <code>Date</code>.
	 * 
	 * @param row
	 *            the table row to set
	 * @param field
	 ...
	 * operation creates an entirely new table independent of this table. If a
	 * filtered view of this same table is preferred, use the
	 * {@link CascadedTable} class.
	 * 
	 * @param filter
	 *            the predicate filter determining which rows to include in the
	 *            new table. If this value is null, all rows will be included.
	 * @param sort
	 *            the sorting criteria determining the order in which rows are
	 *            added to the new table. If this value is null, the rows will
	 *            not be sorted.
	 ...
	 * 
	 * @param filter
	 *            a predicate specifying which rows to remove from the table.
	 */
	 ...
	 * field. This method will create an index over the field if one does not
	 * yet exist.
	 * 
	 * @param field
	 *            the data field to sort by
	 * @param ascend
	 *            true if the iteration should proceed in an ascending (lowest
	 *            to highest) sort order, false for a descending order
	...
	/**
	 * Return an iterator over a range of rwos in this table, determined by a
	 * bounded range for a given data field. A new index over the data field
	 * will be created if it doesn't already exist.
	 * 
	 * @param field
	 *            the data field for determining the bounded range
	 * @param lo
	 *            the minimum range value
	 * @param hi
	 *            the maximum range value
	 * @param indexType
	 *            indicate the sort order and inclusivity/exclusivity of the
	 *            range bounds, using the constants of the
	 *            {@link prefuse.data.util.Index} class.
	 * @return an iterator over a range of table rows, determined by a sorted
	 *         bounded range of a data field
	 */
	 ...
	 *            the modified column
	 * @param start
	 *            the starting row of the modified range
	 * @param end
	 ...
	 *            operations effecting all columns.
	 * @param type
	...
	protected static class ColumnEntry {
		...
		/**
		 * Dispose of this column entry, disposing of any allocated metadata or
		 * index instances.
		 */
		public void dispose() {
			if (metadata != null)
