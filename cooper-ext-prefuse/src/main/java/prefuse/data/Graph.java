...
/**
 * <p>
 * A Graph models a network of nodes connected by a collection of edges. Both
 * nodes and edges can have any number of associated data fields. Additionally,
 ...
 * is just an artifact of the order in which the nodes were specified when added
 * to the graph.
 * </p>
 * 
 * <p>
 * Both nodes and edges are represented by backing data {@link Table} instances
 * storing the data attributes. For edges, this table must also contain a source
 * node field and a target node field. The default column name for these fields
 * are {@link #DEFAULT_SOURCE_KEY} and {@link #DEFAULT_TARGET_KEY}, but these
 * can be configured by the graph constructor. These columns should be integer
 * valued, and contain either the row number for a corresponding node in the
 * node table, or another unique identifier for a node. In this second case, the
 * unique identifier must be included as a data field in the node table. This
 * name of this column can be configured using the appropriate graph
 * constructor. The default column name for this field is
 ...
 * outlinks for all nodes.
 * </p>
 * 
 * <p>
 * Graph nodes and edges can be accessed by application code by either using the
 * row numbers of the node and edge tables, which provide unique ids for each,
 * or using the {@link prefuse.data.Node} and {@link prefuse.data.Edge} classes
 * -- {@link prefuse.data.Tuple} instances that provide object-oriented access
 * to both node or edge data values and the backing graph structure. Node and
 * Edge tuples are maintained by special TupleManager instances contained within
 * this Graph. By default, they are not accessible from the backing node or edge
 ...
 * over Node or Edge tuples, not the backing data tables.
 * </p>
 * 
 * <p>
 * All Graph instances also support an internally generated spanning tree,
 ...
 * underlying tree structure to compute a graph layout.
 * </p>
 * 
...
public class Graph extends CompositeTupleSet {
	...
	protected void init(Table nodes, Table edges, boolean directed, String nodeKey, String sourceKey, String targetKey) {
		// sanity check
		if ((nodeKey != null && !TypeLib.isIntegerType(nodes.getColumnType(nodeKey)))
	 ...
	 * 
	 * @param ntm
	 *            the TupleManager to use for nodes
	 * @param etm
	...
	public void setTupleManagers(TupleManager ntm, TupleManager etm) {
		if (!Node.class.isAssignableFrom(ntm.getTupleType()))
			throw new IllegalArgumentException("The provided node "
					+ "TupleManager must generate tuples that implement " + "the Node interface.");
		if (!Edge.class.isAssignableFrom(etm.getTupleType()))
			throw new IllegalArgumentException("The provided edge "
					+ "TupleManager must generate tuples that implement " + "the Edge interface.");
		m_nodeTuples = ntm;
		m_edgeTuples = etm;
	}
	 ...
	 *            true if this method should throw an Exception when an invalid
	 *            node is encountered
	 ...
	 * there are multiple edges between two nodes; in those cases this method
	 ...
	 *            an edge id (edge table row number)
	 * @param node
	 *            a node id (node table row number). This node id must be
	 ...
	 *            an Edge instance
	 * @param n
	 *            a Node instance. This node must be connected to the edge
	 ...
	 *            {@link #INEDGES} (for in-linking edges), {@link #OUTEDGES}
	 *            (for out-linking edges), or {@link #UNDIRECTED} (for all
	 *            edges).
	 * @return an iterator over all edge ids for edges incident on the given
	 *         node
	 */
	public IntIterator edgeRows(int node, int direction) {
		if (direction == OUTEDGES) {
			...
			return new CompositeIntIterator(edgeRows(node, OUTEDGES), edgeRows(node, INEDGES));
		} else {
			throw new IllegalArgumentException("Unrecognized edge type: " + direction
					+ ". Type should be one of Graph.OUTEDGES, " + "Graoh.INEDGES, or Graph.ALL");
		}
	}
	...
	public Iterator tuples(Predicate filter) {
		if (filter == null) {
			...
			return new CompositeIterator(m_edgeTuples.iterator(getEdgeTable().rows(filter)),
