 ...
 * 
 * <p>
 * The AcivityManager runs in its own separate thread of execution, and one
 * instance is used to schedule activities from any number of currently active
 * visualizations. The class is implemented as a singleton; the single instance
 * of this class is interacted with through static methods. These methods are
 * called by an Activity's run methods, and so are made only package visible
 * here.
 * </p>
 * 
 * <p>
 ...
 * {@link prefuse.activity.Activity#runAfter(Activity)} methods. These will
 * automatically call the appropriate methods with the ActivityManager.
 * </p>
 * 
 * <p>
 * For {@link prefuse.action.Action} instances, one can also register the
 * actions with a {@link prefuse.Visualization} and use the visualizations
 * provided run methods to launch Actions in a convenient fashion. The
 * interface, which is backed by an {@link ActivityMap} instance, also provides
 * a useful level of indirection, allowing actions to be changed dynamically
 * without changes to code in other locations.
 * </p>
 * 
...
public class ActivityManager extends Thread {
	...
	/**
	 * Stops the activity manager thread. All scheduled actvities are canceled,
	 * and then the thread is then notified to stop running.
	 */
	...
	/**
	 * Schedules an Activity to start immediately, overwriting the Activity's
	 * currently set startTime.
	 * 
	 * @param a
	 *            the Activity to schedule
	 */
	...
	/**
	 * Schedules an Activity to start immediately after another Activity. The
	 * second Activity will be scheduled to start immediately after the first
	 * one finishes, overwriting any previously set startTime. If the first
	 * Activity is cancelled, the second one will not run.
	 * 
	 ...
	 * finish of the first.
	 * 
	 * This method does not otherwise effect the scheduling of the first
	 * Activity.
	 * 
	 * @param before
	 *            the first Activity to run
	...
	/**
	 * Cancels an Activity and removes it from this manager, called by an
	 * Activity when the activity needs to be cancelled.
	 * 
	 * @param a
	...
	/**
	 * Removes an Activity from this manager, called by an Activity when it
	 * finishes or is cancelled. Application code should not call this method!
	 * Instead, use Activity.cancel() to stop a sheduled or running Activity.
	 * 
	 * @param a
	 * @return true if the activity was found and removed, false if the activity
	 *         is not scheduled with this manager.
	 */
	...
	/**
	 * Used by the activity loop to determine if the ActivityManager thread
	 * should keep running or exit.
	 */
