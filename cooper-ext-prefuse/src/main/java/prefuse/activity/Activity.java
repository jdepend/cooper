...
/**
 * Represents an activity that can be scheduled and run. This could include data
 * processing, animation, and time-sensitive operations.
 * 
...
public abstract class Activity {
	 ...
	 * 
	 * @param duration
	 *            the length of this activity. A value of {@link #INFINITY}
	 *            indicates an infinite running time.
	 ...
	 *            indicates an infinite running time.
	 * @param stepTime
	 *            the delay time between steps of this activity
	 * @see prefuse.activity.Activity#Activity(long, long, long)
	 */
	 ...
	 *            the delay time between steps of this activity
	 * @param startTime
	 *            the time at which this activity should begin
	 */
	...
	/**
	 * Schedules this Activity to start immediately after another Activity. This
	 * Activity will be scheduled to start immediately after the first one
	 * finishes, overwriting any previously set startTime. If the first Activity
	 * is cancelled, this one will not run.
	 * 
	 ...
	 * Activity. If the first Activity is never scheduled, this Activity will
	 * correspondingly never be run unless scheduled by a separate scheduling
	 * call.
	 * 
	 * @param before
	 *            the Activity that must finish before this one starts
	 */
	 ...
	 * ActivityManager -- outside code should have no need to call or override
	 * this method. To implement custom activities, override the run() method
	 * instead.
	 * 
	 * @param currentTime
	 *            the time at which this step is being run.
	 * @return the time (in milliseconds) when this activity should be run
	 *         again. A return value of -1 indicates this activity is finished.
	 */
	...
	/**
	 * Cancels this activity, if scheduled. This will stop a running activity,
	 * and will remove the activity from the ActivityManager's schedule.
	 */
	public void cancel() {
		/*
		 * Prefuse Bug ID #1708926 The fix ("Contribution") has not been tested
		 * and/or validated for release as or in products, combinations with
		 * products or other commercial use. Any use of the Contribution is
		 * entirely made at the user's own responsibility and the user can not
		 * rely on any features, functionalities or performances Alcatel-Lucent
		 * has attributed to the Contribution. THE CONTRIBUTION BY
		 * ALCATEL-LUCENT (...) IS PROVIDED AS IS, WITHOUT WARRANTY OF ANY KIND,
		 * EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF
		 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, COMPLIANCE,
		 * NON-INTERFERENCE AND/OR INTERWORKING WITH THE SOFTWARE TO WHICH THE
		 * CONTRIBUTION HAS BEEN MADE, TITLE AND NON-INFRINGEMENT. IN NO EVENT
		 * SHALL ALCATEL-LUCENT (...) BE LIABLE FOR ANY DAMAGES OR OTHER
		 * LIABLITY, WHETHER IN CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT
		 * OF OR IN CONNECTION WITH THE CONTRIBUTION OR THE USE OR OTHER
		 * DEALINGS IN THE CONTRIBUTION, WHETHER TOGETHER WITH THE SOFTWARE TO
		 * WHICH THE CONTRIBUTION RELATES OR ON A STAND ALONE BASIS.
		 */
		ActivityManager.cancelActivity(this);
	}

	/**
	 * Indicates if this activity is currently scheduled with the
	 * ActivityManager
	 * 
	 ...
	 * 
	 * @param s
	 *            the new running state of this activity
	 */
	 ...
	 * 
	 * @param l
	 *            the ActivityListener to remove
	 */
	 ...
	 * position in an animation or other similarly parameterized activity. The
	 * returned value is determined by consulting this Activity's pacing
	 * function.
	 * 
	 * @param elapsedTime
	 *            the time in milliseconds since the start of this Activity.
	 * @return a value between 0 and 1 indicating the current position in an
	 *         animation.
	 ...
	 *         basic, linear pace is used, moving from 0 to 1 uniformly over
	 *         time.
	 */
	...
	/**
	 * Sets the pacing function associated with this Activity. Pacing functions
	 * are used to control the pace of animations.
	 * 
	 * @param pfunc
	 *            this Activity's new pacing function, or null to indicate a
	 *            basic, linear pace moving from 0 to 1 uniformly over time.
	 */
	 ...
	 * 
	 * @return the stopping time for this activity, or Long.MAX_VALUE if this
	 *         activity should run indefinitely.
	 */
	 ...
	 * 
	 * @param duration
	 *            The new duration, in milliseconds, for this activity. A value
	 *            of {@link #INFINITY} indicates that this activity should run
	 *            indefinitely.
	 */
	 ...
	 * 
	 * @param time
	 *            the new starting time for this activity
	 */
	 ...
	 * 
	 * @param time
	 *            the new step time between runs of this activity
	 */
	 ...
	 * 
	 * @param s
	 *            true to enable component, false to disable it
	 */
