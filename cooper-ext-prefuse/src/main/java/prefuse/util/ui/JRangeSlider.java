...
public class JRangeSlider extends JComponent implements MouseListener, MouseMotionListener, KeyListener {
	/*
	 * NOTE: This is a modified version of the original class distributed by Ben
	 * Bederson, Jesse Grosjean, and Jon Meyer as part of an HCIL Tech Report.
	 * It is modified to allow both vertical and horitonal modes. It also fixes
	 * a bug with offset on the buttons. Also fixed a bug with rendering using
	 * (x,y) instead of (0,0) as origin. Also modified to render arrows as a
	 * series of lines rather than as a GeneralPath. Also modified to fix
	 ...
	 * @param orientation
	 *            - construct a horizontal or vertical slider?
	 */
	public JRangeSlider(int minimum, int maximum, int lowValue, int highValue, int orientation) {
		this(new DefaultBoundedRangeModel(lowValue, highValue - lowValue, minimum, maximum), orientation,
				LEFTRIGHT_TOPBOTTOM);
	}
	 ...
	 *            - construct a horizontal or vertical slider?
	 * @param direction
	 *            - Is the slider left-to-right/top-to-bottom or
	 ...
	 * 
	 * @param model
	 *            - a BoundedRangeModel specifying the slider's range
	 * @param orientation
	 *            - construct a horizontal or vertical slider?
	 * @param direction
	 *            - Is the slider left-to-right/top-to-bottom or
	 ...
	 * 
	 * @param g
	 *            a Graphics2D context for rendering
	 * @param width
	 *            the width of the slider trough
	 * @param height
	 *            the height of the slider trough
	 */
	...
	public void paintComponent(Graphics g) {
		...
		if (orientation == VERTICAL) {
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				...
				paintArrow(g2, (width - ARROW_WIDTH) / 2.0, max + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH,
						ARROW_HEIGHT, false);
			} else {
				g2.setColor(getForeground());
				...
				// Draw arrows
				g2.setColor(Color.black);
				paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min + (ARROW_SZ - ARROW_HEIGHT) / 2.0, ARROW_WIDTH,
						ARROW_HEIGHT, false);
			...
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				...
				// Draw arrows
				g2.setColor(Color.black);
				paintArrow(g2, min - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0,
						ARROW_HEIGHT, ARROW_WIDTH, true);
				paintArrow(g2, max + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT,
						ARROW_WIDTH, false);
			} else {
				g2.setColor(getForeground());
				...
				// Draw arrows
				g2.setColor(Color.black);
				paintArrow(g2, min + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT,
						ARROW_WIDTH, true);
				paintArrow(g2, max - ARROW_SZ + (ARROW_SZ - ARROW_HEIGHT) / 2.0, (height - ARROW_WIDTH) / 2.0,
	...
	protected int toScreen(int xOrY) {
		...
		// If the direction is left/right_top/bottom then we subtract the min
		// and multiply times scale
	...
	private void grow(int increment) {
		model.setRangeProperties(model.getValue() - increment, model.getExtent() + 2 * increment, model.getMinimum(),
