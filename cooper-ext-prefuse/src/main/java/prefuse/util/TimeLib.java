...
/**
 * Library routines for dealing with times and time spans. All time values are
 * given as long values, indicating the number of milliseconds since the epoch
 * (January 1, 1970). This is the same time format returned by the
 * {@link java.lang.System#currentTimeMillis()} method.
 * 
 * @author jeffrey heer
 */
public class TimeLib {
	...
	private static final int[] CALENDAR_FIELDS = { Calendar.YEAR, Calendar.MONTH, Calendar.DATE, Calendar.HOUR_OF_DAY,
	 ...
	 * 
	 * @param t0
	 *            the first timestamp (as a long)
	 * @param t1
	 *            the second timestamp (as a long)
	 * @param field
	 *            the time unit to use, one of the {@link java.util.Calendar}
	...
	private static int estimateUnitsBetween(long t0, long t1, int field) {
		...
		case Calendar.MILLISECOND:
			return (int) d; // this could be very inaccurate. TODO: use long
							// instead of int?
	 ...
	 *            {@link java.util.Calendar} fields, or one of the extended
	 *            fields provided by this class (MILLENIUM, CENTURY, or DECADE).
	 * @param val
	 *            the number of time units to increment by
	 */
	 ...
	 * {@link java.util.Calendar#get(int)} method, but include support for the
	 * extended fields provided by this class (MILLENIUM, CENTURY, or DECADE).
	 * 
	 * @param c
	 *            the Calendar
	 * @param field
	 ...
	 *            the year to look up
	 * @param month
	 *            the month to look up (months start at 0==January)
	 * @param day
	 *            the day to look up
	...
	/**
	 * Get a timestamp for the given hour, minute, and second. The date will be
	 * assumed to be January 1, 1970.
	 * 
	 * @param c
	 *            a Calendar to use to help compute the result. The state of the
	 *            Calendar will be overwritten.
	 ...
	 *            the reference time
	 * @param c
	 *            a Calendar instance used to help compute the value, the state
	 *            of the Calendar will be overwritten.
	 * @param field
	 ...
	 * These multi-year spans are the extended fields introduced by this class
	 * (MILLENIUM, CENTURY, and DECADE).
	 * 
	 * @param field
	 *            the time field
