...
public class MathLib {
	...
	/**
	 * The "safe" base 10 logarithm of the input value, handling negative values
	...
	/**
	 * The "safe" square root of the input value, handling negative values by
	 * simply making them positive and then negating the return value.
	 * 
	 * @param x
	 *            the input value
	...
	/**
	 * Interpolates a value within a range using a specified scale, returning
	 * the fractional position of the value within that scale.
	 * 
	 * @param scale
	 ...
	 * @param val
	 *            the interpolation value, a fraction between 0 and 1.0.
	 * @param dist
	 *            a double array describing the distribution of the data. For
	 *            the {@link prefuse.Constants#QUANTILE_SCALE} option, this
	 *            should be a collection of quantile boundaries, as determined
	 *            by the {@link #quantiles(int, double[])} method. For any other
	 *            scale type, the first value of the array must contain the
	 *            minimum value of the distribution and the last value of the
	 *            array must contain the maximum value of the distribution; all
	 *            values in between will be ignored.
	 * @return the fractional position of the value within the scale, a double
	 *         between 0 and 1.
	 */
	...
	/**
	 * Interpolates a value between a given minimum and maximum value using a
	 * linear scale.
	 * 
	 * @param val
	 *            the interpolation value, a fraction between 0 and 1.0.
	...
	/**
	 * Compute the n-quantile boundaries for a set of values. The result is an
	 * n+1 size array holding the minimum value in the first entry and then n
	 * quantile boundaries in the subsequent entries.
	 * 
	 * @param n
	 *            the number of quantile boundaries. For example, a value of 4
	 *            will break up the values into quartiles, while a value of 100
	 *            will break up the values into percentiles.
	 * @param values
	 *            the array of double values to divide into quantiles
	 * @return an n+1 array of doubles containing the minimum value and the
	 *         quantile boundary values, in that order
	 */
	...
	/**
	 * Get the quantile measure, as a value between 0 and 1, for a given value
	 * and set of quantile boundaries. For example, if the input value is the
	 * median of the distribution described by the quantile boundaries, this
	 * method will return 0.5. As another example, if the quantile boundaries
	 * represent percentiles, this value will return the percentile ranking of
	 * the input value according to the given boundaries.
	 * 
	 * @param val
	 *            the value for which to return the quantile ranking
	 * @param quantiles
