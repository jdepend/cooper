...
public class IOLib {
	 ...
	 * @param s
	 *            the string to check
	 * @return true if a url string matching the listed protocols, false
	 *         otherwise
	 */
	 ...
	 * may point to an Internet location (e.g., http:// or ftp:// URL), a
	 * resource on the class path (resulting in a resource URL that points into
	 * the current classpath), or a file on the local filesystem (resulting in a
	 * file:// URL). The String will be checked in that order in an attempt to
	 * resolve it to a valid URL.
	 * 
	 * @param location
	 ...
	 * may point to an Internet location (e.g., http:// or ftp:// URL), a
	 * resource on the class path (resulting in a resource URL that points into
	 * the current classpath), or, if the <code>includeFileSystem</code> flag is
	 * true, a file on the local filesystem (resulting in a file:// URL). The
	 * String will be checked in that order in an attempt to resolve it to a
	 * valid URL.
	 * 
	 * @param location
	 *            the location string for which to get a URL object
	 * @param referrer
	 *            the class to check for classpath resource items, the location
	 *            string will be resolved against the package/folder containing
	 *            this class
	 * @param includeFileSystem
	 *            indicates if the file system should be included in the search
	 *            to resolve the location String
	 * @return a URL object, or null if the location string could not be
	 *         resolved
	 */
	 ...
	 * requested from the URL connection. If this fails, the location will be
	 * resolved against the file system. Also, if a gzip file is found, the
	 * input stream will also be wrapped by a GZipInputStream. If the location
	 * string can not be resolved, a null value is returned
	 * 
	 * @param location
	 *            the location string
	...
	/**
	 * Indicates if the given file ends with a file extension of ".gz" or ".Z",
	 * indicating a GZip file.
	 * 
	 * @param file
	...
	/**
	 * Indicates if the given file ends with a file extension of ".zip",
	 * indicating a Zip file.
	 * 
	 * @param file
	 *            a String of the filename or URL of the file
	 ...
	 * {@link #streamFromString(String)} method, then read the input stream into
	 * a String result.
	 * 
	 * @param location
	 *            the location String
