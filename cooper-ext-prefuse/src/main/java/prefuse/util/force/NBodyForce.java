 ...
 * <ul>
 * <li><a
 * href="http://www.cs.berkeley.edu/~demmel/cs267/lecture26/lecture26.html"
 * >James Demmel's UC Berkeley lecture notes</a>
 * <li><a
 * href="http://www.physics.gmu.edu/~large/lr_forces/desc/bh/bhdesc.html">
 * Description of the Barnes-Hut algorithm</a>
 * <li><a
...
public class NBodyForce extends AbstractForce {
	...
	private void forceHelper(ForceItem item, QuadTreeNode n, float x1, float y1, float x2, float y2) {
		...
		if ((!n.hasChildren && n.value != item) || (!same && (x2 - x1) / r < params[BARNES_HUT_THETA])) {
			...
			for (int i = 0; i < n.children.length; i++) {
				if (n.children[i] != null) {
					forceHelper(item, n.children[i], (i == 1 || i == 3 ? splitx : x1), (i > 1 ? splity : y1), (i == 1
							|| i == 3 ? x2 : splitx), (i > 1 ? y2 : splity));
				}
			}
			if (minDist)
				return;
