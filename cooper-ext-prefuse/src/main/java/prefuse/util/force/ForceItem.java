...
public class ForceItem implements Cloneable {
	...
	/**
	 * Checks a ForceItem to make sure its values are all valid numbers (i.e.,
	 * not NaNs).
	 * 
	 * @param item
	 *            the item to check
	...
	public static final boolean isValid(ForceItem item) {
		return !(Float.isNaN(item.location[0]) || Float.isNaN(item.location[1]) || Float.isNaN(item.plocation[0])
				|| Float.isNaN(item.plocation[1]) || Float.isNaN(item.velocity[0]) || Float.isNaN(item.velocity[1])
