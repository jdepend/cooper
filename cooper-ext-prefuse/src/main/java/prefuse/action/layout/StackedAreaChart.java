...
public class StackedAreaChart extends Layout {
	...
	public void setOrientation(int orient) {
		...
		m_horiz = (m_orientation == Constants.ORIENT_LEFT_RIGHT || m_orientation == Constants.ORIENT_RIGHT_LEFT);
		m_top = (m_orientation == Constants.ORIENT_TOP_BOTTOM || m_orientation == Constants.ORIENT_LEFT_RIGHT);
	}
	...
	public void run(double frac) {
		...
		int mult = m_top ? 1 : -1;
		float inc = (float) (m_horiz ? (bounds.getMinY() - bounds.getMaxY()) : (bounds.getMaxX() - bounds.getMinX()));
		inc /= columns.length - 1;
		int len = columns.length;

		// perform first walk to compute max values
		double maxValue = getPeaks();
		float b = (float) (m_horiz ? (m_top ? bounds.getMinX() : bounds.getMaxX()) : (m_top ? bounds.getMinY() : bounds
				.getMaxY()));
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < columns.length; ++i) {
				...
				baseline[i] += mult * hgt * MathLib.linearInterp(value, 0, peaks[i]);
				poly[base + xbias] = min + i * inc;
				poly[base + ybias] = (float) baseline[i];
				height = Math.max(height, Math.abs(poly[2 * (len - 1 - i) + ybias] - poly[base + ybias]));
			}
	...
	private float[] getPolygon(VisualItem item, String field) {
		...
		if (poly == null || poly.length < 4 * columns.length) {
			...
			inc /= len - 1;
			float max = (float) (m_horiz ? (m_top ? bounds.getMaxX() : bounds.getMinX()) : (m_top ? bounds.getMinY()
					: bounds.getMaxY()));
			float min = (float) (m_horiz ? bounds.getMaxY() : bounds.getMinX());
