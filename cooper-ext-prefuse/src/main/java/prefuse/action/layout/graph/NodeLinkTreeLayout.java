 ...
 * 
 * <p>
 * The algorithm used is that of Christoph Buchheim, Michael J黱ger, and
 * Sebastian Leipert from their research paper <a
 * href="http://citeseer.ist.psu.edu/buchheim02improving.html"> Improving
...
public class NodeLinkTreeLayout extends TreeLayout {
	...
	public void setOrientation(int orientation) {
		if (orientation < 0 || orientation >= Constants.ORIENTATION_COUNT || orientation == Constants.ORIENT_CENTER) {
	...
	private double spacing(NodeItem l, NodeItem r, boolean siblings) {
		boolean w = (m_orientation == Constants.ORIENT_TOP_BOTTOM || m_orientation == Constants.ORIENT_BOTTOM_TOP);
		return (siblings ? m_bspace : m_tspace)
				+ 0.5
				* (w ? l.getBounds().getWidth() + r.getBounds().getWidth() : l.getBounds().getHeight()
						+ r.getBounds().getHeight());
	}

	private void updateDepths(int depth, NodeItem item) {
		boolean v = (m_orientation == Constants.ORIENT_TOP_BOTTOM || m_orientation == Constants.ORIENT_BOTTOM_TOP);
		double d = (v ? item.getBounds().getHeight() : item.getBounds().getWidth());
