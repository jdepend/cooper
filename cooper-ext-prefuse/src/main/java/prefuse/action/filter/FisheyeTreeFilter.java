...
/**
 * <p>
 * Filter Action that computes a fisheye degree-of-interest function over a tree
 * structure (or the spanning tree of a graph structure). Visibility and DOI
 * (degree-of-interest) values are set for the nodes in the structure. This
 * function includes current focus nodes, and includes neighbors only in a
 * limited window around these foci. The size of this window is determined by
 * the distance value set for this action. All ancestors of a focus up to the
 * root of the tree are considered foci as well. By convention, DOI values start
 * at zero for focus nodes, with decreasing negative numbers for each hop away
 * from a focus.
 * </p>
 * 
 * <p>
 * This form of filtering was described by George Furnas as early as 1981. For
 * more information about Furnas' fisheye view calculation and DOI values, take
 * a look at G.W. Furnas, "The FISHEYE View: A New Look at Structured Files,"
 * Bell Laboratories Tech. Report, Murray Hill, New Jersey, 1981. Available
 * online at <a href="http://citeseer.nj.nec.com/furnas81fisheye.html">
...
public class FisheyeTreeFilter extends GroupAction {
	 ...
	 * 
	 * @param group
	 *            the data group to process. This should resolve to a Graph
	 *            instance, otherwise exceptions will result when this Action is
	 *            run.
	 */
	 ...
	 *            run.
	 * @param distance
	 *            the graph distance threshold from high-interest nodes past
	 *            which nodes will not be visible nor expanded.
	 */
	 ...
	 *            run.
	 * @param sources
	 *            the group to use as source nodes, representing the nodes of
	...
	/**
	 * Get the graph distance threshold used by this filter. This is the
	 * threshold for high-interest nodes, past which nodes will not be visible
	 * nor expanded.
	 * 
	...
	/**
	 * Set the graph distance threshold used by this filter. This is the
	 * threshold for high-interest nodes, past which nodes will not be visible
	 * nor expanded.
	 * 
	 * @param distance
	 ...
	 * distance. These form the roots from which the graph distance is measured.
	 * 
