...
/**
 * <p>
 * Assignment Action that assigns size values for a group of items based upon a
 * data field. This action can be used to automatically vary item's on screen
 ...
 * (length) and 2D (area) encodings are supported by this function. 2D is
 * assumed by default; use the setIs2DArea method to change this.
 * </p>
 * 
 * <p>
 * The size assignments for numerical data are continuous by default, but can be
 * binned into a few discrete steps (see {@link #setBinCount(int)}).
 ...
 * this can sometimes result in very large differences. For example, if the
 * minimum data value is 1.0 and the largest is 200.0, the largest items will be
 * 200 times larger than the smallest. While accurate, this may not result in
 * the most readable display. To correct these cases, use the
 * {@link #setMaximumSize(double)} method to manually set the range of allowed
 * sizes. By default, the minimum size value is 1.0. This too can be changed
 * using the {@link #setMinimumSize(double)} method.
 * </p>
 * 
...
public class DataSizeAction extends SizeAction {
	 ...
	 *            {@link prefuse.Constants#QUANTILE_SCALE}. If a quantile scale
