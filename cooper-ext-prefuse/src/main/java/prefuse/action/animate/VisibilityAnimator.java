...
/**
 * Animator that interpolates the visibility status of VisualItems. Items not
 * currently visible but with end visibilty true are faded in, while items
 * currently visible but with end visibility false are faded out and finally set
 * to not visible.
 * 
	...
	private void setup() {
		...
		while (items.hasNext()) {
			...
			if (!item.isStartVisible()) {
				item.setStartFillColor(ColorLib.setAlpha(item.getEndFillColor(), 0));
				item.setStartStrokeColor(ColorLib.setAlpha(item.getEndStrokeColor(), 0));
				item.setStartTextColor(ColorLib.setAlpha(item.getEndTextColor(), 0));
			}
		}
		...
		while (items.hasNext()) {
			...
			if (!item.isEndVisible()) {
				...
				item.setEndFillColor(ColorLib.setAlpha(item.getStartFillColor(), 0));
				item.setEndStrokeColor(ColorLib.setAlpha(item.getStartStrokeColor(), 0));
				item.setEndTextColor(ColorLib.setAlpha(item.getStartTextColor(), 0));
			}
		}
	}
