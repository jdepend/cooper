 ...
 * {@link prefuse.Visualization#putAction(String, Action)} to do this. This will
 * ensure that the Action is configured to use that Visualization. If an Action
 * is part of an {@link ActionList} or {@link ActionSwitch}, it is sufficient to
 * only register that CompositeAction with the Visualization -- all contained
 * Action instances will be configured appropriately. You can then run the
 * Actions using the {@link prefuse.Visualization#run(String)} method and other
 * similar methods of the {@link prefuse.Visualization} class.
 * </p>
 * 
 * <p>
 * As a subclass of Activity, Actions can be of two kinds. <i>Run-once</i>
 * action lists have a duration value of zero, and simply run once when
 * scheduled. Actions with a duration greater than zero can be executed multiple
 * times, waiting a specified step time between each execution until the
 * activity has run for its full duration. A duration of Activity.INFINITE will
 * result in a continually re-running Action.
 * </p>
 * 
...
public abstract class Action extends Activity {
	...
	private final static Logger s_logger = Logger.getLogger(Action.class.getName());

	/** A reference to the visualization processed by this Action. */
	protected Visualization m_vis;

	/**
	 * Creates an action instance with zero duration. This Action will only run
	 * once if invoked.
	 */
	public Action() {
		this(null);
	 ...
	 * 
	 * @param vis
	 *            the Visualization this Action should process. If this Action
	 *            is registered with another Visualization, this value will be
	 *            overwritten.
	 */
	...
	/**
	 * Create a new Action with a specified Visualization, duration and step
	 * time.
	 * 
	 * @param vis
	 *            the Visualization this Action should process. If this Action
