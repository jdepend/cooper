...
/**
 * Re-implementation of Ben Fry's Zipdecode. Check out the original at <a
 * href="http://acg.media.mit.edu/people/fry/zipdecode/">
...
public class ZipDecode extends Display implements Constants {
	...
	public ZipDecode(final Table t) {
		...
		Predicate filter = (Predicate) ExpressionParser.parse("state >= 1 && state <= 56 && state != 2 && state != 15");
		...
		Predicate loneResult = (Predicate) ExpressionParser
				.parse("INGROUP('_search_') AND GROUPSIZE('_search_')=1 AND " + "LENGTH(QUERY('_search_'))=5");
