...
public class Congress extends JPanel {
	...
	public Congress(Table t) {
		...
		vt.addColumn("label", "CONCAT(CAP(Candidate), ' (', " + "CAP([Party Designation]), '-', [State Code], "
		...
		int[] palette = new int[] { ColorLib.rgb(150, 150, 255), ColorLib.rgb(255, 150, 150),
				ColorLib.rgb(180, 180, 180) };
		DataColorAction color = new DataColorAction(group, "Party", Constants.ORDINAL, VisualItem.STROKECOLOR, palette);
	...
	private class Counter extends GroupAction {
		...
		public void run(double frac) {
			...
			if (count == 1) {
				...
				m_totalStr = count + " Candidates receiving " + NumberFormat.getCurrencyInstance().format(total);
			}
			m_total.setText(m_totalStr);
		}
	}
