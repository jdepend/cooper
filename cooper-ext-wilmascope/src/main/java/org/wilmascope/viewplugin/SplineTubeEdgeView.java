...
public class SplineTubeEdgeView extends EdgeView implements View2D, org.wilmascope.dotlayout.Spline {
	...
	void splineTube(Point2D.Float[] controlPoints, BranchGroup b) {
		...
		for (int n = 0; n < numCurves; n++) {
			...
			// so we recalculate again as follows (one strip for each tube
			// segment):
	...
	public void draw2D(Renderer2D r, Graphics2D g, float transparency) {
		 ...
		 * getAppearance().getMaterial().getDiffuseColor(c); if(c.x>0.5f) {
		 * g.setStroke(new BasicStroke(thickness, BasicStroke.CAP_ROUND,
		 * BasicStroke.JOIN_ROUND, 0f, new float[]{0,6,0,6}, 0.0f)); } else
		 * if(c.y>0.5f) { g.setStroke(new BasicStroke(thickness,
		 * BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, new
		 * float[]{5.0f}, 0.0f)); } else g.setColor(new
		 * Color(c.x,c.y,c.z,transparency));
		 */
